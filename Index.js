...
client.on('interactionCreate', async (i) => {
    ...
    if (i.commandName === 'addpalavra') {
        ...
        let adicionadas = [];
        palavras.forEach(p => {
            if (!serverConfig.palavrasBloqueadas.includes(p)) {
                serverConfig.palavrasBloqueadas.push(p);
                adicionadas.push(p);
            }
        });

        if (adicionadas.length === 0) {
            return await i.editReply('⚠️ **Aviso:** Todas as palavras enviadas já constavam no filtro.');
        }
        return await i.editReply(`🚫 **Filtro Atualizado:** Adicionadas \`${adicionadas.length}\` novas palavras.\n📝 **Termos:** ${adicionadas.map(p => `\`${p}\``).join(', ')}`);
    }

    if (i.commandName === 'clear') {
        const qtd = i.options.getInteger('quantidade');
        const del = await i.channel.bulkDelete(qtd, true).catch(() => []);
        return await i.editReply(`🧹 **Chat Limpo:** Foram removidas \`${del.size || del.length || 0}\` mensagens.`);
    }
...
// LÓGICA DE DETECÇÃO CORRIGIDA, ULTRA PRECISA E ANTI-FALHAS
client.on('guildMemberAdd', async (member) => {
    // 1. Pega todas as variações de nomes possíveis da conta de forma segura
    const username = (member.user.username || "").toLowerCase();
    const displayName = (member.user.displayName || "").toLowerCase();
    
    // 2. Tenta extrair o status customizado (Bio/Presença) tratando erros de cache do Discord
    let statusTexto = "";
    if (member.presence && member.presence.activities) {
        const atividadeCustomizada = member.presence.activities.find(a => a.type === 4); // Tipo 4 = Status Customizado
        if (atividadeCustomizada && atividadeCustomizada.state) {
            statusTexto = atividadeCustomizada.state.toLowerCase();
        }
    }

    // Junta tudo em um único texto para varredura completa
    const textoCompletoDoPerfil = `${username} | ${displayName} | ${statusTexto}`;

    // 3. Executa o teste contra a lista de palavras banidas por Raid
    const alvoDetectado = TERMOS_PROIBIDOS_PERFIL.some(termo => textoCompletoDoPerfil.includes(termo));

    if (alvoDetectado) {
        try {
            // Bane o usuário imediatamente do servidor
            await member.ban({ reason: '🚨 Filtro Automático Avançado: Conta com padrões de perfil ou bio associados a Raids.' });
            
            // Dispara o relatório estético no canal global do desenvolvedor
            const alertaDev = new EmbedBuilder()
                .setTitle('🚨 MONITORAMENTO: TENTATIVA DE RAID MANDADA PRO ESPAÇO')
                .setColor('#FF0000')
                .addFields(
                    ...
                    { name: '📝 Texto Identificado:', value: `\`\`\`${textoCompletoDoPerfil}\`\`\`` }
                )
                .setTimestamp();
            await enviarAlertaGlobal(alertaDev);
        } catch (err) {
            console.error("Erro ao tentar banir conta de raid: ", err);
        }
    }
});
...
client.on('messageCreate', async (m) => {
    ...
    if (serverConfig.antilink) {
        ...
        if (regexLink.test(m.content)) {
            try {
                await m.delete();
                const avisoL = await m.channel.send(`🔗 <@${m.author.id}>, o compartilhamento de links externos está bloqueado.`);
