...
const LINK_SERVIDOR_SUPORTE = 'https://discord.gg/zyphor'; 
...
function gerarPainelSeguranca() {
    const embed = new EmbedBuilder()
        ...
        .addFields(
            ...
            { name: '🤫 Proteção Anti-Spam', value: serverConfig.antispam ? '🟩 **ATIVADO**' : '🟥 **DESATIVADO**', inline: true }
        )
        .setFooter({ text: 'Proteção em tempo real ativa.' });

    const row = new ActionRowBuilder().addComponents(
        new ButtonBuilder().setCustomId('tg_link').setLabel('Anti-Link').setEmoji('🔗').setStyle(serverConfig.antilink ? ButtonStyle.Danger : ButtonStyle.Success),
        new ButtonBuilder().setCustomId('tg_raid').setLabel('Anti-Raid').setEmoji('🚨').setStyle(serverConfig.antiraid ? ButtonStyle.Danger : ButtonStyle.Success),
        new ButtonBuilder().setCustomId('tg_spam').setLabel('Anti-Spam').setEmoji('🤫').setStyle(serverConfig.antispam ? ButtonStyle.Danger : ButtonStyle.Success)
    );
...
client.on('interactionCreate', async (i) => {
    ...
    if (i.commandName === 'suporte') {
        const embedSuporte = new EmbedBuilder()
            .setTitle('🎫 Central de Ajuda & Suporte')
            .setDescription(`Precisa de auxílio técnico, deseja reportar falhas ou tirar dúvidas sobre o projeto?\n\nClique no botão abaixo para ingressar em nossa comunidade oficial de suporte:\n\n➡️ **[Servidor de Suporte Oficial](${LINK_SERVIDOR_SUPORTE})**`)
            .setColor('#5865F2')
...
client.on('messageCreate', async (m) => {
    ...
    if (temPermissao(m.member)) return; // Ignora administradores e cargos configurados no /setup
