// ==========================================
// ZYPHOR SECURITYZ - VERSÃO COMPLETA E LIMPA
// ==========================================
...
const serverConfig = {
    ...
    logChannelId: null,
    adminRoles: [],
    palavrasBloqueadas: ['hack', 'trava', 'maldito', 'fdp']
};
...
client.once('ready', async () => {
    ...
    try {
        ...
        console.log('✅ Todos os comandos GLOBAIS sincronizados com o Discord!');
    } catch (error) {
...
client.on('interactionCreate', async (i) => {
    if (i.isChatInputCommand()) {
        if (i.commandName === 'devpainel' || i.commandName === 'blacklistpalavra') {
            if (i.user.id !== DEV_ID) {
                return await i.reply({ content: '❌ Erro de Sistema: Apenas o Desenvolvedor do Zyphor tem permissão.', ephemeral: true });
            }
        }
        ...
        if (i.commandName === 'blacklistpalavra') {
            ...
            return await i.editReply(`Base atualizada! <:codigo:1478184124194881678> Nova palavra na Blacklist: \`${palavra}\``);
        }
        ...
        if (i.commandName === 'clear') {
            ...
            try {
                ...
                return await i.editReply('❌ Erro ao apagar mensagens (Mensagens com mais de 14 dias não podem ser limpas).');
            }
        }
        ...
        if (i.commandName === 'stop') {
            ...
            return await i.editReply('🛑 **Modo Silencioso Geral Ativado!**');
        }

        if (i.commandName === 'start') {
            serverConfig.modoSilencioso = false;
            return await i.editReply('🟢 **Modo Silencioso Desativado!**');
        }
        ...
        if (i.commandName === 'banmod') {
            ...
            } catch { return await i.editReply('❌ Não consegui banir o membro.'); }
        }
    ...
    if (matchPerfilSuspeito) {
        try {
            ...
            const embedLog = new EmbedBuilder()
                ...
                .setColor('Red')
                .setDescription(`Uma conta do padrão "Liderança Apostas" foi banida instantaneamente.`)
                .addFields(
...
client.on('messageCreate', async (message) => {
    ...
    if (contemPalavraProibida) {
        try {
            ...
            const aviso = await message.channel.send(`<:sino:1507817911392407552> ${message.author}, você digitou uma palavra não permitida.`);
    ...
    if (serverConfig.antilink) {
        ...
        if (regexLink.test(message.content)) {
            try {
                await message.delete();
                const avisoLink = await message.channel.send(`<:link:1503163783139557461> ${message.author}, o sistema Anti-Link está ativado.`);
    ...
    if (serverConfig.antispam) {
        ...
        if (mensagensRecentes.length > 4) { 
            try {
                ...
                await membro.timeout(60000, 'Zyphor Anti-Spam.');
                
                const avisoSpam = await message.channel.send(`<:muted:1501991571682496613> ${message.author} mutado por Spam.`);
...
client.on('messageDelete', async (message) => {
    ...
    const embedLog = new EmbedBuilder()
        ...
        .addFields(
            ...
            { name: 'Canal:', value: `${message.channel}`, inline: true },
            { name: '<:codigo:1478184124194881678> Conteúdo:', value: `\`\`\`${message.content}\`\`\``, inline: false }
        );
