...
// 📌 CONFIGURAÇÃO PROTEGIDA (Apenas o Token do Bot)
...
client.on('guildCreate', async (guild) => {
    if (!servidoresAutorizados.has(guild.id)) {
        ...
        try {
            ...
            if (canal) {
                await canal.send(`<:erro:1508472500495974600> **Zyphor Security v3** é um sistema privado. Contate a equipe para autorizar seu servidor.`);
            }
        } catch (e) {}
        await guild.leave(); 
    } else {
...
// -------------------------------------------------------------------------
// COMANDO /ADCUSUARIO SIMPLIFICADO (Livre para o servidor usar)
// -------------------------------------------------------------------------
...
client.on('ready', async () => {
    ...
    const comandos = [
        new SlashCommandBuilder()
            .setName('adcusuario')
            .setDescription('📥 Autoriza um novo servidor para usar o Zyphor através do link.')
